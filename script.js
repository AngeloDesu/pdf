...
async function savePO() {
  ...
  try {
    ...
    console.error('Error saving PO (local):', err);
    alert('Error saving PO locally. See console for details.');
  }
}
...
async function renderList(mod) {
  ...
  try {
    // read from localStorage instead of fetching from server
...
async function downloadRPFromDB(id) {
  try {
    console.log('Loading RP (local) with ID:', id);
    const rp = findById('rp', id);
    if (!rp) throw new Error('RP not found');
    console.log('RP data:', rp);
    
    if (!rp || !rp.no) {
      throw new Error('Invalid RP data received');
    }
...
// async function downloadRPFromDB(id) {
//   try {
//     console.log('Fetching RP with ID:', id);
...
async function savePR() {
  ...
  try {
    ...
    console.error('Error saving PR (local):', err);
    alert('Error saving PR locally. See console for details.');
  }
}
...
//     document.getElementById("pr-date").value = pr.date || '';
...
//     if (Array.isArray(items) && items.length > 0) {
//       items.forEach(it => {
//         prAddItem(it.stk || '', it.qty || 0, it.unit || '', it.desc || it.description || '', it.remark || it.remarks || '');
//       });
//     }
...
//   } catch (err) {
//     console.error('downloadPRFromDB Error:', err);
//     alert('Error loading Purchase Requisition: ' + err.message);
//   }
// }
...
async function saveRP() {
  ...
  try {
    ...
    if (editingRPId) {
      ...
      rp.id = editingRPId;
      // if invoice_image is present (data URL) it will overwrite previous; if user cleared, keep existing
      const existing = list[idx] || {};
      if (!rp.invoice_image && existing.invoice_image) {
        // keep old image unless explicitly cleared
    ...
    console.error("Error saving RP (network):", err);
    alert("Error saving RP to database. See console for details.");
  }
}
